...
until curl -s http://localhost:9200/_cluster/health | grep -qE '"status":"green"|"status":"yellow"'; do
    sleep 2
done

echo "Elasticsearch siap! Membuat index security-alerts..."

curl -X PUT "http://localhost:9200/security-alerts" -H 'Content-Type: application/json' -d'
{
  "mappings": {
    "properties": {
      "timestamp": { "type": "date" },
...
curl -X POST "http://localhost:9200/security-alerts/_bulk" -H 'Content-Type: application/json' -d'
{ "index":{} }
